...
public class Password {
    ...
    public static void main(String[] args){
        ...
        System.out.println(T.solution(n, password));
    }
    ...
    public String solution(int n, String password){
        ...
        for(int i=0;i<n;i++){
            strs[i] = password.substring(0+i*7,7+i*7);
            strs[i] = strs[i].replace('#','1');
            strs[i] = strs[i].replace('*','0');
            ints[i] = Integer.parseInt(strs[i], 2);
            ans += String.valueOf(Character.toChars(ints[i]));
        }
